...
>
> **Última atualização:** **Dashboards, medição editável e as cinco respostas da cliente.** O Planejamento Inteligente (F0→F6) e a priorização (P1 + config) já estavam no ar; este arco entregou: **Dashboard Projetos completo** na tela Prioridades (`fc3437e`) depois de um **bench derrubar a premissa de sumarização** da spec (`22f5d9e`); a **medição mensal editável** (`6105a50`, `78271af`) — bloqueio ativo da cliente; e as **cinco respostas dela**: macro "Geral" automática (`8913c93`), aviso de vigência no grid (`1399605`), fixar/pausar na priorização (`ebcd960`, `d391605`). Mais a **`Spec_UI_Dashboards.md`** (`74bca9d`), sintetizada de um brainstorm de 4 IAs sobre a FORMA dos dashboards — a spec anterior definia os dados, não o desenho, e o Projetos tinha virado uma tabela de 13 colunas sem cara de painel. Tudo em `feat/alocacao-fase-2`. Próximo: implementar a UI dos dashboards conforme a spec nova (Projetos refeito → Capacidade → Geral) — ver §7.
>
...
| **Pós-demo (1ª leva)** | Preparo do demo, custos, programas de fomento, **tarifa por colaborador × categoria**, regra de priorização, **dashboards** — ver §4-bis, §4-sexies e §4-decies | 🟡 Custos + programas + **tarifa** ✅; **priorização P1 + config + fixar/pausar** ✅; **Dashboard Projetos** ✅ completo (`fc3437e`); **Capacidade e Geral** pendentes (com `Spec_UI_Dashboards.md` pronta) |
...
| **Medição mensal editável** | A medição de cada mês deixa de ser derivada (uniforme) e passa a ser editável com pino; piso `máx(0)` na meta — ver §4-duodecies | ✅ **Completa** (backend `6105a50`, frontend `78271af`) — 2 bordas aguardando decisão da cliente |
...
**No ar e completos:** Planejamento Inteligente (F0→F6), priorização inteira (P1 + config + fixar/pausar), filtro do chefe por gestor, Dashboard Projetos, medição mensal editável, e as cinco respostas da cliente de 18/07.

**PRÓXIMO — a UI dos dashboards** (`Spec_UI_Dashboards.md` pronta, ordem sugerida na §8 dela): refazer a forma do Projetos (fusão 13→8 colunas, faixa de veredito, chips, calibragem de cor) extraindo os componentes compartilhados (`FaixaVeredito`, `Chip`, `BarraCSS`, `fmtRealizado()`) → **Capacidade** → **Geral** → as abas. Retrabalho no Projetos é barato e valida os componentes antes dos outros dois.

**AGUARDANDO A CLIENTE:**
- **As duas bordas da medição editável** (transbordo do oficial; bloquear ou avisar no estouro) — ver §4-duodecies. O sistema avisa e não decide até a resposta.
