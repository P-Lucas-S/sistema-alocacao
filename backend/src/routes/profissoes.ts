...
router.get('/', authenticate, requireRole('admin', 'gestor', 'coordenacao', 'chefe', 'diretor'), async (req: AuthRequest, res) => {
  try {
    ...
    console.error('List profissões error:', error);
...
router.post('/', authenticate, requireRole('admin', 'gestor'), async (req: AuthRequest, res) => {
  try {
    ...
    const todas = await prisma.profissao.findMany();

    // EXATO — ignorando maiúsculas/minúsculas — nunca contornável
    const exata = todas.find(p => p.nome.toLowerCase() === nomeTrim.toLowerCase());
    if (exata) {
      return res.status(409).json({ error: 'Já existe uma profissão com esse nome', existente: exata });
    }

    // SIMILAR — pulado quando confirmarSimilar: true
    if (!confirmarSimilar) {
      const similares = todas.filter(p => isSimilar(p.nome, nomeTrim));
    ...
    if (error?.code === 'P2002') {
      return res.status(409).json({ error: 'Já existe uma profissão com esse nome' });
    }
    console.error('Create profissão error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.patch('/:id', authenticate, requireRole('admin', 'gestor'), async (req: AuthRequest, res) => {
  try {
    ...
    const current = await prisma.profissao.findUnique({ where: { id } });
    if (!current) return res.status(404).json({ error: 'Profissão não encontrada' });
    ...
    if (nome !== undefined) {
      ...
      const outras = await prisma.profissao.findMany({ where: { id: { not: id } } });

      const exata = outras.find(p => p.nome.toLowerCase() === nomeTrim!.toLowerCase());
      if (exata) {
        return res.status(409).json({ error: 'Já existe uma profissão com esse nome', existente: exata });
      }

      if (!confirmarSimilar) {
        const similares = outras.filter(p => isSimilar(p.nome, nomeTrim!));
        if (similares.length > 0) {
          return res.status(200).json({
            needsConfirmation: true,
            similares: similares.map(s => ({ id: s.id, nome: s.nome, ativo: s.ativo })),
    ...
    const updated = await prisma.profissao.update({
      where: { id },
      data: {
    ...
    console.error('Update profissão error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
