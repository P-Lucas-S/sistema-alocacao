...
>
> **Última atualização:** **Spec de papéis — passos 1 a 4 implementados (backend).** Os quatro primeiros dos seis passos da `Spec_Papeis_Posse_Exclusao.md` estão fechados e no GitHub: **(1)** os papéis `chefe` e `diretor` adicionados aos três existentes, com `requireRole` tipado (`cae3015`); **(2)** `criadoPorId` no projeto + delegação na criação — chefe cria já delegando a um gestor (`6861e87`); **(3)** override do chefe nas rotas de operação, com o teto provado sob concorrência chefe×gestor (`c07d6c7`); **(4)** re-delegação — chefe troca o `gestorId` de um projeto + linha de auditoria, sem tocar alocações/histórico (`c5342d4`). Tudo provado por teste de API (papéis 11/11, delegação 16/16, override 15/15 + concorrência 8/8, re-delegação 23/23). **Faltam os passos 5 (exclusão permanente + lápide) e 6 (telas do diretor/dashboards, junto da frente de frontend).** Antes desta leva: bug de macro/micro resolvido (`acb3383`, `21b44ec`), painel na célula vazia (`1a2b7bb`), tarifa por colaborador × categoria (§4-bis). **Fases 0 a 4 + tarifa + passos 1–4 dos papéis completos.** Depois dos passos 5–6: **priorização → dashboards (3) → Fase 5** — ver §7. A tela de histórico do log (E2-b) segue como extra deferido.
...
| **Pós-demo (2ª leva — Manual)** | Papéis novos (chefe/diretor) + delegação + exclusão permanente, bug macro/micro, e itens menores — ver §4-ter | 🟡 **Bug macro/micro ✅** (`acb3383`, `21b44ec`) + painel na célula vazia (`1a2b7bb`); **spec de papéis: passos 1–4 ✅** (`cae3015`, `6861e87`, `c07d6c7`, `c5342d4`), **passos 5–6 pendentes** |
| Transversal | Identidade visual geral | ✅ Reforma clara aplicada no preparo do demo (`c378876`, `db44f99`) |

> **Fases 0–4 + tarifa + passos 1–4 dos papéis fechados.** A **2ª leva de pedidos** (via o `Manual`) tem o núcleo estrutural — **papéis chefe/diretor, delegação, override e re-delegação** — **implementado** (passos 1–4, §4-ter), com o bug de macro/micro já corrigido (`acb3383`, `21b44ec`) e o arrumo no grid (`1a2b7bb`). O que falta na spec de papéis: **passo 5** (exclusão permanente + lápide) e **passo 6** (telas do diretor/dashboards + a frente de frontend do chefe). Depois dela: a **priorização** (regra fechada), os **dashboards (3)**, e a **Fase 5**.
...
- **`GET /api/categorias` não inclui `chefe` no `requireRole`** (hoje `admin`/`gestor`/`coordenacao`) — **achado no passo 4**. O chefe cria/delega projetos e a criação exige escolher um programa, então a **tela de criação do chefe** (passo 6) vai precisar listar categorias e hoje tomaria 403. Não quebra nada agora (a tela do chefe ainda não existe); correção de uma linha, a fazer junto do frontend do chefe ou num ajuste de `requireRole` de leitura. *(No `test-redelegacao` foi contornado buscando a categoria com token de gestor.)*
...
Com as **Fases 0 a 4 + a tarifa completas** e os **passos 1–4 da spec de papéis fechados** (§4-ter), a fila é:

1. **Spec de papéis — terminar os passos 5 e 6.** Os passos 1–4 (papéis, `criadoPorId`/delegação, override do chefe, re-delegação) **estão feitos** (`cae3015`, `6861e87`, `c07d6c7`, `c5342d4`; detalhe no §4-ter). **O próximo a começar é o passo 5 — exclusão permanente + lápide** (status `pendente_exclusao` + bloqueio por alocação + cascata explícita micros→macros→prestações→projeto + tabela-lápide `projeto_excluido` + recusa de alocação em projeto pendente + código liberado pra reuso). Tem migration de verdade e várias decisões → vai no esquema de sempre (5a: schema + migration mostrada antes de aplicar; 5b: rotas de solicitar/aprovar/recusar/excluir-direto + teste). O **passo 6** (telas do diretor + a frente de frontend do chefe) casa com os **dashboards** (item 3 abaixo). *(Pendências anotadas pro caminho: o `requireRole` de `GET /categorias` pro chefe, e a decisão de UX do remanejamento do chefe — ver fila do §4-ter.)*
