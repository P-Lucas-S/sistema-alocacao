...
import { Briefcase, Plus, Pencil, Power, PowerOff, X, AlertTriangle } from 'lucide-react';
...
export default function Areas() {
  ...
  const fetchAreas = useCallback(async () => {
    try {
      const res = await fetch('/api/areas-atuacao', {
  ...
  async function handleCriar(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { setCriarErro(data.error || 'Erro ao criar área'); return; }

      // 201 — criada (objeto vem em data.area)
      setNovoNome('');
      fetchAreas();
    } catch {
  ...
  async function handleCriarConfirmar() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setCriarErro(data.error || 'Erro ao criar área'); setCriarPending(null); return; }
  ...
  async function handleRenomear(id: string) {
    ...
    setEditSaving(true);
    try {
      const res = await fetch(`/api/areas-atuacao/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      ...
      // sucesso — objeto vem DIRETO (não dentro de { area })
      setEditId(null);
      setEditNome('');
      fetchAreas();
    } catch {
  ...
  async function handleRenomearConfirmar() {
    ...
    setEditErro('');
    try {
      const res = await fetch(`/api/areas-atuacao/${editPending.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  async function toggleAtivo(a: Area) {
    setTogglingId(a.id);
    try {
      await fetch(`/api/areas-atuacao/${a.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    ...
    <div className="p-6 flex flex-col gap-6 h-full overflow-y-auto">
      ...
      <div>
        ...
        <p className="text-sm mt-0.5" style={{ color: 'var(--text-3)' }}>
          Gerencie as áreas de atuação disponíveis para os colaboradores.
        </p>
      </div>
          ...
          <div className="flex items-start gap-2 p-3 rounded-xl" style={{ background: 'hsl(38 92% 50% / 0.1)', border: '1px solid hsl(38 92% 50% / 0.3)' }}>
            ...
            <div className="flex-1">
              <p className="text-sm" style={{ color: '#b45309' }}>
                Já existe uma área parecida: <strong>{criarPending.similares.map(s => s.nome).join(', ')}</strong>. Cadastrar mesmo assim?
              </p>
            ...
            >
              {editId === a.id ? (
                <>
                  ...
                  {editPending && editPending.id === a.id && (
                    <div className="flex items-start gap-2 p-3 rounded-xl" style={{ background: 'hsl(38 92% 50% / 0.1)', border: '1px solid hsl(38 92% 50% / 0.3)' }}>
                      <AlertTriangle size={15} className="shrink-0 mt-0.5" style={{ color: '#b45309' }} />
                      <div className="flex-1">
                        <p className="text-sm" style={{ color: '#b45309' }}>
                          Já existe uma área parecida: <strong>{editPending.similares.map(s => s.nome).join(', ')}</strong>. Renomear mesmo assim?
                        </p>
                        <div className="flex gap-3 mt-3">
                          <button
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-lg"
                      style={ghostBtnStyle}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = a.ativo ? '#b42318' : '#15803d'; (e.currentTarget as HTMLElement).style.background = 'var(--surface-2)'; }}
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = 'var(--text-3)'; (e.currentTarget as HTMLElement).style.background = 'transparent'; }}
                    >
                      {a.ativo ? <><PowerOff size={11} /> Desativar</> : <><Power size={11} /> Ativar</>}
                    </button>
                  </div>
                </div>
              )}
