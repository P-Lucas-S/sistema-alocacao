...
| **Pós-demo (1ª leva)** | Preparo do demo, custos, programas de fomento, **tarifa por colaborador × categoria**, regra de priorização, **dashboards** — ver §4-bis, §4-sexies, §4-decies e §4-terdecies | ✅ **Completa** — custos + programas + tarifa; priorização inteira; **os 3 dashboards** (Projetos `fc3437e`+`f387a93`+`3d4159a`, Geral `bbf4d42`, Capacidade `9e56ed8`) com a visão do diretor (`8f8ebcd`, `a51a1af`) |
| **Pós-demo (2ª leva — Manual)** | Papéis novos (chefe/diretor) + delegação + exclusão permanente, bug macro/micro, e itens menores — ver §4-ter | 🟡 **Bug macro/micro ✅** (`acb3383`, `21b44ec`) + painel na célula vazia (`1a2b7bb`); **spec de papéis: passos 1–5 ✅** (`cae3015`, `6861e87`, `c07d6c7`, `c5342d4`, `2e807d6`, `af2cbb3`), **passo 6 ✅** (as telas do diretor = a visão dele nos dashboards, `8f8ebcd`/`a51a1af` — spec de papéis COMPLETA); **itens menores ✅**: Perfil tema claro (`be7848b`), GET /categorias pra chefe+diretor (`4f08a44`), Cargo em Custos (`0dc3cc7`) |
...
## 4-decies. Dashboards — o bench que derrubou a sumarização e o Dashboard Projetos completo

**O bench (`22f5d9e`).** A `Spec_Dashboards.md` afirma (consenso unânime das 4 IAs) que **tabelas de sumarização pré-agregadas são obrigatórias**, porque agregar 200 projetos × 200 colaboradores em tempo real "derrubaria o banco" — e a versão síncrona recomendada **tocaria a transação com o lock do teto de 220h**, o invariante mais sensível do sistema. Antes de assumir esse risco, **medimos**: `bench-dashboards.mjs` popula volume realista (prefixo `BENCH-`, idempotente, com limpeza) e cronometra as 3 agregações.

**Resultado: a sumarização é desnecessária.** Pior caso **58ms** (Projetos, todos os gestores); caso comum (gestor filtrando os próprios) **31ms**; Capacidade **4,6ms**. E o **achado principal**: o **teto de 220h LIMITA o volume** — com 200 colaboradores em ~6 projetos por mês, são **~1.200 linhas de alocação por mês**, não centenas de milhares; chegar a 100k linhas exigiria cada pessoa em ~83 projetos simultâneos, o que é irreal. **A premissa da spec estava errada.** Os dashboards agregam sob demanda, como a meta, a priorização e o relatório de custos já fazem. O script fica versionado como evidência da decisão e para permitir remedir se a escala mudar.

**Dashboard Projetos completo (`fc3437e`).** A investigação mostrou que **a tela Prioridades JÁ ERA o Dashboard Projetos** (o endpoint dela é `/api/dashboards/projetos`; ~70% de sobreposição com o que a spec pedia). Decisão: **completar, não criar** — uma segunda tela duplicaria o fetch, os seletores e a tabela, confundiria com dois itens de menu parecidos, e perderia a engrenagem de config. Nome mantido ("Prioridades"): já existe uma tela "Projetos" no menu. Adicionados 4 campos (`categoriaNome`; `gestorNome`; `custoRealizado` — acumulador paralelo no mesmo loop, zero query nova; `qtdColabsGargalo` — o `Map<boolean>` virou `Map<number>`, com o `sinalCapacidade` booleano preservado como derivado, então o contrato do GET não quebrou) e 4 colunas (Programa com badge, Gestor — oculta para o papel gestor, % Exec., Custo real.). **Regra da spec respeitada:** o % só aparece com apontamento; sem ele, **"N/D", nunca "0%"**.

**A forma visual — `Spec_UI_Dashboards.md` (`74bca9d`).** O Projetos ficou correto mas **sem cara de painel**: uma tabela de 13 colunas. A `Spec_Dashboards.md` define os **dados**, quase nada da **forma**. Um brainstorm de 4 IAs sobre desenho de interface foi sintetizado numa spec de UI, com as decisões arbitradas: **camada de veredito** (agregados no topo, universal) sobre **camada de evidência** (linhas, recortadas por papel); **cor sinaliza estado, não classificação** (categoria Alta é classificação e pode ser comum — badge mudo; vencido/≥95% é exceção — vermelho); **fusão de colunas por conceito** (13→8: "Execução" = barra + "312/480h · 65%" numa célula); **zero biblioteca de gráficos** (tudo `div` com width); **abas** numa tela só em vez de três itens de menu (trocar de aba não pode resetar o mês); **`fmtRealizado()` centralizado** — o único lugar onde `null` vs `0` se decide, tornando a regra do "N/D" impossível de vazar; barra sem realizado é **contorno vazio**, não 0% preenchido. Ordem sugerida: refazer o Projetos → extrair os componentes → Capacidade → Geral.

---

## 4-undecies. Priorização — fixar/pausar (fecha a P2)

Último item da P2 (ver `Regra_Priorizacao_reconstruida.md` §5), **com uma correção da cliente**.

**O documento estava ERRADO.** A regra reconstruída dizia que fixar leva o projeto ao "topo da lista inteira, acima de todas as categorias". A cliente corrigiu (**resposta 1B**): fixar leva ao **topo da CATEGORIA dele** — um Média fixado é o primeiro dos Médios e continua **abaixo de todos os Altas**. A categoria de prazo segue soberana; o fixar só desempata dentro dela. **Foi exatamente por isso que a implementação foi segurada até perguntar** — o doc foi reconstruído de memória e este era o ponto marcado como incerto. A ordenação virou: categoria → fixado antes de não-fixado → horas pendentes desc.

**Pausar (resposta 2B):** o projeto sai da fila e vai para uma **seção "Pausados" ao fim** — não some (a opção "some da lista" foi a rejeitada). Pausado não recebe categoria e **não entra nas contagens por categoria** (pausar significa tirar do radar; contar mesmo assim tornaria a ação cosmética), mas a **contagem de pausados vem separada**, senão os números encolheriam sem explicação. Fixar e pausar são mutuamente exclusivos por construção (cada rota zera a outra flag).

**Backend (`ebcd960`).** Migration aditiva: dois booleanos no projeto (default `false`, então todos os existentes ficam idênticos — portão garantido no nível do banco) e a tabela **`prioridade_logs`** para auditoria (quem, quando, qual projeto, qual das 4 ações). Tabela própria em vez de reusar `AlocacaoLog`, que é acoplada a alocações e deixaria metade das colunas nulas; segue o padrão da `Redelegacao` (`projetoId` sem FK para o log sobreviver à exclusão do projeto). 4 rotas PATCH com permissão em duas camadas: `requireRole` barra coordenação/diretor, e um guard manual barra gestor em projeto alheio (403). Portão: priorização 26/26, dashboard 17/17, config 19/19, filtro 30/30; `test-fixar-pausar` 39/39 — **com o caso que separa** a correção da cliente do documento (montado com projetos em categorias diferentes, senão os dois comportamentos coincidem).

**Frontend (`d391605`).** O `GET /dashboards/projetos` passa a devolver `{ itens, pausados, totalPausados }` — antes era array só com ativos, e com a Fase A no ar um pausado **sumia da tela**, justamente a opção rejeitada pela cliente. Pin **sempre visível** ao lado do badge marca os fixados (informação é para todos); os botões de ação ficam numa coluna à direita, só para quem pode agir. Seção "Pausados" abaixo da tabela, esmaecida, quando há algum. **Ajuste após a validação:** os botões estavam só no hover, pequenos e apagados — "uma ação que só existe no hover é uma ação que o gestor precisa adivinhar que existe". Passaram a sempre visíveis em tom neutro, seguindo o padrão dos botões de editar/apagar macro (mesmo tamanho e tratamento de cor), para a tela não ganhar um dialeto próprio de botão.

---

## 4-duodecies. Medição mensal editável (pino de medição)

**O pedido (18/07).** A medição de cada mês era **derivada** (`valorTotal ÷ nº de meses`, uniforme) e a cliente precisa definir *"o valor exato que vou pagar"* em cada mês. Era **bloqueio ativo**: ela tentou contornar pinando a Meta HT de todos os meses, o que produzia déficits falsos (o pino sobrescreve o cálculo, então a meta não caía a zero mesmo com o oficial cobrindo tudo). Ver `Spec_Medicao_Editavel.md`.

**Backend (`6105a50`).** **Pino de medição** reusando a mecânica do pino da meta (que ela já conhece): `medição = pinoMedição ?? (valorTotal − soma dos pinos) ÷ nº de meses não pinados`, com o último não-pinado absorvendo o resíduo → soma exata ao centavo. **Tabela própria** (`medicao_mensal_ajustes`) em vez de tornar `meta_ht` nullable na tabela existente: preservar o `NOT NULL` mantém a garantia estrutural de que "linha existente ⟹ tem pino de meta" — o pino da meta está no ar e o motor de sugestão consome o déficit que sai dele. **Piso na meta:** `metaHT = máx(0, medição − oficial)` (antes não havia piso porque o oficial nunca excedia a medição por construção). **O caso da cliente resolvido:** medição 51.000 com oficial 61.538 → meta **0** e déficit **0**.

**Efeito em cadeia corrigido:** o `cascataPendente` assumia que `soma(metaHT) = valorHT` **sempre fecha** — com o piso cortando valor isso pode não fechar legitimamente, e dispararia falso positivo. A condição saiu do `cascataPendente` (que agora cobre só problemas de cascata) e o gap passa a ser reportado por **`totalCortadoPeloPiso` em R$** — mais informativo que um booleano. A estratégia `'inicial'` do oficial **depende das medições**, então redistribui junto quando há pinos (coberto por teste). Portão: meta 46/46, cascata 87/87, motor 108/108, pino-meta 27/27; `test-pino-medicao` 64/64.

**Frontend (`78271af`).** A coluna Medição vira editável com a mesma interação da Meta HT. Os dois pinos podem coexistir no mesmo mês e são distinguíveis por **posição e rótulo** ("pin meta" na coluna Mês, "pin med." na célula Medição). Os três avisos, cada um na sua altitude: o piso por mês (ícone com tooltip na célula), o estouro do projeto (banner no topo), e o total cortado no mini-resumo — sem ele a gestora veria os números divergindo e acharia que é bug.

**AS DUAS BORDAS AGUARDAM A CLIENTE** (perguntadas em 19/07, sem resposta até agora). O sistema **avisa e não decide**:
1. **Oficial excede a medição do mês** → meta zera pelo piso e sobra oficial não aproveitado. Ele **transborda** para o mês seguinte (a fala dela sugere isso: *"o planejador apropria nos próximos meses HT"*) ou **fica parado**? Não implementar transbordo até confirmar — mudaria a distribuição do oficial, que é lógica validada.
2. **Pinos de medição somando acima do `valorTotal`** → **bloquear** a edição ou **avisar**? Hoje avisa (não-pinados em 0), sem bloquear nem reduzir os pinos dela. A validação mostrou que o estado de estouro produz números estranhos (soma das metas bem acima do "A Apropriar"), o que é argumento para bloquear.

---

## 4-terdecies. Os três dashboards — a UI da spec inteira no ar

**Fases 1a/1b — o Projetos refeito** (`f387a93`, `3d4159a`). A tabela de 13 colunas virou **8 por fusão de conceitos** (Execução = mini-barra + "312/480h · 65%"; Custo = planejado + realizado; Equipe = "6 · 2⚠"; código sob o nome; programa na expansão), com a **calibragem de cor** ("cor sinaliza estado, não classificação": badge de categoria mudo — dot + texto neutro; vermelho preenchido só pra violação; borda esquerda 3px só em vencido, transparente nas demais pra alinhamento), o **`fmtRealizado()` centralizado** (o único lugar da decisão null-vs-zero; barra sem realizado = contorno vazio, nunca 0%), e a **faixa de veredito** + chips-filtro (a faixa reflete o escopo e IGNORA filtros — é o sumário do todo; KPI zerado renderiza mudo). Correção de contraste pós-validação: `--text-3` rende ~3.2:1 no tema escuro (vs 4.7:1 no claro) — chevron, dot e código foram pra `--text-2`; os travessões FICAM em `--text-3` de propósito (ausência deve ser quieta).

**Decisão de navegação (diverge da spec de UI, que propunha abas):** o **Início = Dashboard Geral** (a tela estava vazia desde sempre — fora pensada pra isso), **Prioridades = Dashboard Projetos**, **Capacidade = item novo**. Itens de menu separados; o problema que as abas resolviam (trocar de aba não pode resetar o mês) se resolve com o mês em contexto compartilhado, e itens são mais descobríveis.

**Dashboard Geral** (`bbf4d42`). Sem tabela, nunca: 5 KPIs, bloco Prazo (barra + chips → `/prioridades?categoria=X`, lazy initializer), bloco Apontamento. **Backend:** `headcountAlocado` e `emSobrecarga` calculados por **UNION de IDs** (não soma por projeto — um colaborador em 3 projetos conta 1; teste com cenário capaz de distinguir). **Correção da métrica de apontamento** achada na validação: media **cobertura de projetos** ("N de M com ≥1h") em vez da **taxa de horas** da spec — um gestor caiu de 20% pra 1% com a métrica certa (120h planejadas sem apontamento entravam no denominador); o ranking tinha o mesmo bug. Sem horas planejadas → travessão, nunca 0%.

**Dashboard Capacidade** (`9e56ed8`). `GET /dashboards/capacidade` reusa o `calcularPriorizacao` (+1 query só de nomes). **A nuance do escopo:** quem aparece = colabs dos projetos do gestor; as **horas são GLOBAIS** (o teto de 220h é global — fatia parcial daria ocupação falsa; mesmo critério do Saldo 220h do grid). E a correção de revisão: o **realizado ia ser parcial** (só escopo), criando inconsistência na mesma linha — resolvido sem query extra (`horasRealizadas` no select da query global, acúmulo no mesmo loop). 3 chips-filtro (Risco ≥95% · Saudável 50–94% · Ocioso <50%, limiares 209h/110h) + barra empilhada pra liderança (não histograma: com 3 faixas ele degenera); tabela por ocupação desc; barra própria (limiares 50/95 ≠ 80/100 do grid).

**Visão do diretor** (`8f8ebcd`) **e a consolidação** (`a51a1af`). Primeiro: em Prioridades o diretor trocou a tabela por blocos agregados ("adaptar por omissão, nunca por bloqueio" — a tabela vaza informação pela forma); chips como texto plano, nunca link morto; o Apontamento dele ganhou o mesmo número grande com cor do gestor (a taxa é agregada, não expõe ninguém — só o ranking com nomes é vedado). Depois, com o payload recortado (§4-quindecies), as três telas dele ficaram rasas — e a decisão foi **consolidar numa tela única**: o Início do diretor tem os 5 blocos (KPIs, Prazo, **Custo por categoria**, **Ocupação da equipe**, Apontamento); Prioridades e Capacidade saem do menu pra ele (as rotas seguem como fallback). Extraídos `KpiCard` e `Bloco` pra `components/` (eram duplicados — item 4.4 da auditoria adiantado). **Isto fecha o passo 6 dos papéis.**

**Seed `seed-dashboards.mjs`** (`e860348`, prefixo `DASH-`, independente dos `CEN-`): categorias em todas as faixas (com vencidos), perfis de apontamento por gestor (83/50/20% — o ranking tem o que ordenar), capacidade nas 3 faixas (2 sobrecarregados, um cross-gestor), 5 programas, fixado+pausado, dados em Jun/Jul/Ago 2026. **Sem ele os dashboards não são validáveis** (tudo zerado não prova nada).

## 4-quaterdecies. Transbordo do oficial (fecha as bordas da medição)

Resposta da cliente: *"o que sobra do oficial passa para o outro mês"*. A investigação provou que a estratégia **'inicial' JÁ ERA transbordo por construção** (waterfall: `oficial = min(saldo, medição)` — o piso nunca dispara nela); o trabalho ficou só na **'proporcional'**, que divide igual e é cega às medições. Um laço com `carry` após a divisão uniforme (`7d724ad`). O transbordo também **corrigiu uma inconsistência real**: sem ele, `soma(metaHT)` não fechava com o `valorHT` quando o piso cortava (485k vs 480k no cenário de teste). Invariante `soma(oficial) = valorOficial` exato preservado; carry sobrando após o último mês (dado inválido) acumula lá com o aviso sinalizando. Testes T4/T5/T6 atualizados **com raciocínio aritmético** (T5: o mês 3 continua preso — não há mês seguinte, correto). Cenários `CEN-OFICIAL`/`CEN-OFICIAL-INI` no seed. A outra borda (estouro dos pinos) ficou como estava por escolha dela ("pode deixar como está, somente o sistema avisando"). **Aberto:** o arredondamento da HT (ela sugeriu "cobrir a medição e ultrapassar em 500–1000"; pergunta com as opções A/B/C enviada — `Pergunta_Juliana_arredondamento.md`).

## 4-quindecies. Auditoria pente-fino e as correções (Fases A, A½, B)

**Motivação:** o Início ficou meses como placeholder sem ninguém notar. Um pente-fino (rodado no **Fable 5** — leitura ampla é onde ele rende) produziu a **`AUDITORIA_CODIGO.md`** (no repo): 7 categorias, com destaque pra **permissões** (9 achados) e um **fluxo órfão de exclusão de projetos** (5 endpoints sem nenhuma UI — o "caso Início" invertido). Regra do relatório: relatar sem corrigir, cada item com onde/impacto/custo.

**Fase A** (`25e81a7`) — os GETs sem `requireRole`: `/colaboradores` (expunha **valorHora e e-mail** de todos ao diretor), `/alocacoes/grid`, `/alocacoes`, `/projetos*`, `/macros` → fechados pra `admin/gestor/chefe/coordenacao`. O `/users` completo virou admin-only; um **`/users/gestores` enxuto** (id+nome) atende o SeletorGestor — acessível ao diretor por decisão explícita (nome de gestor é hierarquia conhecida; o dado protegido é colaborador/tarifa). Dois testes com falha pré-existente corrigidos junto (**regra nova: teste vermelho "conhecido" corrói o portão** — uma falha nova se esconde entre as velhas).

**Fase A½** (`71cf921`) — o furo que a Fase A revelou: os `/dashboards/*` entregavam as **listas completas** ao diretor; só a tela escondia. Agora, quando `role === 'diretor'`, os endpoints devolvem **só os agregados** que a tela dele usa (sem `itens`/`pausados`/`colaboradores`); demais papéis, payload byte-idêntico. `test-seguranca-gets` 51/51 com três classes de assert: ausência dos arrays, **busca por vazamento de string** (nomes do payload do chefe não podem aparecer no JSON do diretor — pega vazamento acidental), e **equivalência numérica** (os agregados do diretor batem com os derivados da resposta do chefe). **A regra do diretor vale de ponta a ponta.**

**Fase B** (`8cc1e04`) — os 403 misteriosos (a família do bug do Custos), com o princípio "o chefe age em tudo": Remanejamento liberado ao chefe (5 endpoints); Programas/Profissões → `admin/chefe/gestor`; **Grid pra coordenação** em leitura real (`readonly` por papel nas 3 ocorrências + o **comentário mentiroso** do Layout corrigido junto + o botão "Copiar plan→real" escondido dela — ponta achada no mapeamento); candidatos liberados a chefe/coordenação; `canEditProject`/`canWrite` alinhados ao backend (chefe edita — o `ProjetoDetalhe` não estava no relatório e foi achado na execução). `test-fase-b` 20/20; `test-chefe-override` ganhou fixture idempotente (15/15 em duas execuções seguidas).

**Pendentes da auditoria:** **Fase C** — decisão de produto sobre o fluxo de exclusão órfão (perguntar à cliente: aprovação de exclusão deve existir?); **Fase D** — polimento em lote: o erro engolido do `Projetos.tsx` (único funcional), `Alocacoes.tsx` órfão, os GETs sem consumidor, os formatters/CATS duplicados, o `gestorNomeVis` de `itens[0]`, o "R$ 0" vs "R$ —", o `noUnusedLocals`, e a dependência de ordem do `test-fase-b` (soft-delete → deleteMany).
...
- **O fluxo de exclusão de projetos** (Fase C da auditoria) — aprovação de exclusão deve existir? 5 endpoints prontos sem UI.
- **A matriz meses×colaboradores no grid geral** — o pedido mais forte dela, mas ela mesma não sabe a forma ("um filtro? na tela de Ver Entregas?"). Merece protótipo, não pergunta fechada. O filtro do chefe (§4-nonies) já entrega parte.
...
**RESOLVIDO NESTE ARCO** (eram pendências da §7 anterior): o quick-win do **boot-timestamp no `/api/health`** (feito em `0f99c47`); a **macro "Geral" automática** (resposta 3A, `8913c93`); o **aviso de vigência no grid** (resposta 4B, `1399605`) junto da correção do **texto obsoleto do grid vazio**; o **fixar/pausar** (respostas 1B/2B, §4-undecies). A **prorrogação de prestações** foi dispensada pela cliente (resposta 5B: ela edita a data da prestação).

**PERGUNTAS ANTIGAS PENDENTES:** se o papel Consulta (ex-coordenacao) será usado; rótulos de exibição dos papéis (Coordenação/Consulta/Gerência — DECIDIDOS, não implementados).
...
- **`Spec_UI_Dashboards.md`** (v1.0) — a FORMA visual dos 3 dashboards (síntese de brainstorm de 4 IAs): camada de veredito sobre camada de evidência, cor sinaliza estado e não classificação, fusão de colunas por conceito, zero biblioteca de gráficos, abas, `fmtRealizado()` centralizado.
- **`Spec_Medicao_Editavel.md`** — o pino de medição, o piso `máx(0)`, as duas bordas pendentes de decisão da cliente e os efeitos em cadeia.
